...
from util import dataset_normal, transform, config
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if main_process():
        ...
        wandb.init(entity='myersdj', project='seg_pytorch', sync_tensorboard=True)
    ...
    train_data = dataset_normal.SemData(split=args.fold, data_root=args.data_root, \
    ...
    if args.evaluate:
        ...
        val_data = dataset_normal.SemData(split=args.fold, data_root=args.data_root, \
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (input, target, mask) in enumerate(train_loader):
        ...
        output, main_loss, aux_loss = model(input, target, mask=mask)
...
def validate(val_loader, model, criterion):
    ...
    for i, (input, target, mask) in enumerate(val_loader):
        ...
        output = model(input)
        if args.zoom_factor != 8:
            output = F.interpolate(output[mask], size=target.size()[1:][mask], mode='bilinear', align_corners=True)

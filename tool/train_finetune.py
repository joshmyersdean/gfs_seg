...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.arch == 'psp':
        ...
        modules_new = [model.psa, model.cls]
    params_list = []
    ...
    #train_data = dataset_normal.SemData(split='train', data_root=args.data_root, data_list=args.train_list, transform=train_transform)
    if args.distributed:
    ...
    if args.evaluate:
        val_transform = transform.Compose([
            ...
            transform.ToTensor(),
            transform.Normalize(mean=mean, std=std)])
        #val_data = dataset_normal.SemData(split='val', data_root=args.data_root, data_list=args.val_list, transform=val_transform)
        val_data = dataset_finetune.SemData(split=args.fold, shot=args.shot, data_root=args.data_root, \
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.evaluate and epoch % args.evaluate_freq == 0:
            loss_val, mIoU_val, mIoU_base, mIoU_novel, mAcc_val, allAcc_val = validate(val_loader, model, criterion, [0]+sub_list, sub_val_list)
            if main_process():
            ...
            if mIoU_val > best_IoU and main_process():
                best_IoU = mIoU_val
                filename = args.save_path + '/train_' + 'best' + '.pth'
                logger.info('Saving checkpoint to: ' + filename)
                torch.save({'epoch': epoch_log, 'mIoU': mIoU_val, 'mIoU_base': mIoU_base, 'mIoU_novel': mIoU_novel, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()}, filename)
            if mIoU_novel > best_IoU_novel and main_process():
                best_IoU_novel = mIoU_novel
                filename = args.save_path + '/train_' + 'bestNovel' + '.pth'
                logger.info('Saving checkpoint to: ' + filename)
                torch.save({'epoch': epoch_log, 'mIoU': mIoU_val, 'mIoU_base': mIoU_base, 'mIoU_novel': mIoU_novel, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()}, filename)
...
def validate(val_loader, model, criterion, base_list, novel_list):
    if main_process():
    ...
    return loss_meter.avg, mIoU, mIoU_base, mIoU_novel, mAcc, allAcc
